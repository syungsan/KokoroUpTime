    ...
    public partial class Chapter7 : Page
    {
        ...
        private void ScenarioBack()
        {
            ...
            if (isSceneChanged)
            {
                ...
                this.dataProgress.LatestChapter7Scene = returnedScene;

                using (var connection = new SQLiteConnection(this.initConfig.dbPath))
                {
                    connection.Execute($@"UPDATE DataProgress SET CurrentScene = '{this.dataProgress.CurrentScene}', LatestChapterScene = '{this.dataProgress.LatestChapter7Scene}' WHERE Id = 1;");
                }
            }
        }
